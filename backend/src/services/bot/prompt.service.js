...
import { eventGuestVars, mergeConversationRules } from "../../utils/defaults.js";
...
function listRules(ai) {
  const incoming = Array.isArray(ai?.rules) ? ai.rules : [];
  if (!incoming.length) return "- Sé amable y no presiones al invitado.";
  return mergeConversationRules(incoming).map((rule) => `- ${rule}`).join("\n");
}
...
Flujo (obligatorio):
El primer mensaje ya se envió. No lo reenvíes. En este turno no se llama a usar_plantilla.

Clasifica CADA mensaje del invitado en UNA intención principal (también en el campo intent del JSON de respuesta):
- faq: pregunta sobre el evento (lugar, hora, alberca, vestimenta, estacionamiento, mesa, etc.).
...
Según la intención:
- faq: responde en reply con las Preguntas frecuentes o datos de este evento (lugar, hora, alberca, vestimenta). Si preguntan varias cosas y alguna no existe en el sistema, responde lo que sí tengas y admite honestamente lo que no sin inventar datos ni roles. No actualices el RSVP.
- asistira:
  ...
  * Si la cantidad está dentro del cupo (o si tras advertir el cupo aceptan solo los lugares disponibles): llama actualizar_confirmacion (confirmado si van todos los del cupo, parcial si van menos). En este turno no se llama a usar_plantilla.
  * Cierre en reply: la ventana de 24 horas está abierta, así que el cierre es un mensaje normal. Si las Reglas de conversación de arriba indican explícitamente CÓMO redactar el mensaje cuando el invitado confirma (qué decir, tono, qué mencionar), sigue esa guía y adáptala al invitado.
  * Si NO hay una regla así: escribe un mensaje genérico de confirmación de asistencia, con un cierre breve y natural, y menciona explícitamente cuántas personas quedaron confirmadas.
- no_asistira: llama actualizar_confirmacion con status no_asistira. En este turno no se llama a usar_plantilla.
...
- seguimiento: llama marcar_seguimiento (deja followUpDate en null; el sistema agenda el recontacto a ${indecisoDays}). Responde breve que les escribes de nuevo más adelante. No insistas en un sí o un no.
...
export function buildInstructions({ event, guest, ai, faqs = [] }) {
  ...
  - asistira:
    ...
    * DENTRO DEL CUPO:
      - La ventana de 24 horas está abierta. En este turno no se llama a usar_plantilla. El reply es un mensaje genérico de confirmación de asistencia.
  ...
  - no_asistira:
    ...
    * En "reply": redacción empática, agradece el aviso, no insistas ni dejes preguntas abiertas.
