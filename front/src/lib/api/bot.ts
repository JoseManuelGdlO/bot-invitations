...
export const botApi = {
  ...
  getPlayground: (eventId: string, guestId: string) =>
    api<BotPlaygroundState>(
      `/dev/events/${eventId}/bot/playground?guestId=${encodeURIComponent(guestId)}`,
    ),
  chat: (
    eventId: string,
    body: { guestId: string; message?: string; reset?: boolean },
  ) =>
  ...
  simulateGuest: (conversationId: string, text: string) =>
    api<SimulateGuestReply>(
      `/dev/conversations/${conversationId}/simulate-guest`,
      {
        method: "POST",
