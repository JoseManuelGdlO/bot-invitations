...
const findTimezoneOption = (timezone: string) =>
  EVENT_TIMEZONE_OPTIONS.find(
    (option) => option.value.toLowerCase() === timezone.toLowerCase(),
  );
...
export const resolveTimezoneOptions = (current?: string): TimezoneOption[] => {
  ...
  if (findTimezoneOption(canonical)) {
    return EVENT_TIMEZONE_OPTIONS;
  }
  return [{ value: canonical, label: canonical }, ...EVENT_TIMEZONE_OPTIONS];
};
