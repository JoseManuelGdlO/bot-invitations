      ...
      <div className="space-y-4 p-5">
        ...
        <div className="space-y-1.5 text-xs text-muted-foreground">
          <p className="flex items-center gap-2">
            <CalendarDays className="size-3.5 text-gold" /> {formatDate(event.date)} · {event.time}
          </p>
          <p className="flex items-center gap-2">
            <MapPin className="size-3.5 text-gold" /> {event.venue}
          </p>
          <p className="flex items-center gap-2">
            <Users className="size-3.5 text-gold" /> {s.invitations} invitaciones · {s.people} personas
          </p>
        </div>
        <div>
        ...
        <p className="text-[11px] text-muted-foreground">Faltan {daysUntil(event.date)} días</p>
      </div>
    </Link>
  );
}
