  ...
  head: () => ({
    meta: [
      { title: "Panel general · Alanna Confirmaciones" },
      {
        name: "description",
        content:
          "Todos tus eventos, confirmaciones y conversaciones en un solo panel.",
      },
      {
        property: "og:title",
        content: "Panel general · Alanna Confirmaciones",
      },
      {
        property: "og:description",
        content: "Todos tus eventos y confirmaciones en un panel.",
      },
...
function EventsDashboard() {
  ...
  const upcoming = events.filter(
    (e) => new Date(`${e.date}T12:00:00`) > new Date(),
  ).length;
    ...
    <main className="mx-auto w-full max-w-7xl flex-1 px-5 py-8 md:px-8 md:py-10">
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          value={active}
          hint={`${upcoming} próximos`}
          icon={CalendarCheck2}
        />
        <StatCard
          label="Total de invitados"
          value={s.people}
          hint={`${s.invitations} invitaciones`}
          icon={Users}
        />
        <StatCard
          label="Confirmados"
          ...
          tone="success"
          hint={`${s.progress}% del total`}
          icon={CheckCircle2}
        />
        <StatCard
          label="Confirmaciones pendientes"
          value={s.pending + s.noReply}
          tone="warning"
