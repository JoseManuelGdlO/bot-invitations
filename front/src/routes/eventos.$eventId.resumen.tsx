  ...
  return (
    <main className="mx-auto w-full max-w-7xl flex-1 px-5 py-8 md:px-8">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Invitaciones registradas" value={s.invitations} icon={Users} />
        <StatCard label="Personas invitadas" value={s.people} />
        <StatCard label="Personas confirmadas" value={s.confirmedPeople} tone="success" icon={CheckCircle2} />
        <StatCard label="Pendientes" value={s.pending} tone="warning" icon={Clock} />
        <StatCard label="No asistirán" value={s.rejected} tone="rose" icon={UserRoundX} />
        <StatCard label="Sin respuesta" value={s.noReply} />
        <StatCard label="Conversaciones activas" value={s.active} tone="gold" icon={MessageCircle} />
        <StatCard label="Días restantes" value={event ? daysUntil(event.date) : 0} tone="gold" />
      </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1.4fr]">
        <section className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          ...
          <div className="mt-6 flex flex-col items-center gap-4">
            <ProgressRing value={s.progress} caption={`${s.progress}% de invitados ya confirmaron`} />
            <div className="grid w-full grid-cols-3 gap-3 text-center">
              <div className="rounded-xl bg-success-soft p-3">
                <p className="font-display text-xl text-success">{s.confirmedPeople}</p>
              ...
              <div className="rounded-xl bg-rose p-3">
                <p className="font-display text-xl text-rose-foreground">{s.rejectedPeople}</p>
        ...
        <section className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          ...
          <div className="mt-3 space-y-3">
            {guests
              ...
              .slice(0, 4)
              .map((g) => (
                <div key={g.id} className="rounded-xl border border-border bg-secondary/40 p-3">
