  ...
  head: () => ({
    meta: [
      { title: "Estadísticas · Alanna Confirmaciones" },
      {
        name: "description",
        content:
          "Gráficas de confirmaciones, respuestas y actividad del evento.",
      },
      { property: "og:title", content: "Estadísticas · Alanna Confirmaciones" },
      {
        property: "og:description",
        content: "Gráficas de confirmaciones y actividad del evento.",
      },
...
function Estadisticas() {
  ...
  const donut = [
    ...
    {
      name: "Pendientes",
      value: Math.max(0, s.people - s.confirmedPeople - s.rejectedPeople),
      color: "var(--chart-2)",
    },
  ...
  const timeline = analytics?.timeline?.length
    ...
    : [
        {
          label: "Mensajes iniciales enviados",
          value: guests.filter((g) => g.lastMessage).length,
          at: "Registrado",
        },
        {
          label: "Primeras respuestas recibidas",
          value: guests.filter((g) => g.lastReply).length,
          at: "Registrado",
        },
        {
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1.6fr]">
        <section className="flex flex-col items-center justify-center rounded-2xl border border-border bg-card p-6 shadow-soft">
          ...
          <ProgressRing
            value={s.progress}
            size={160}
            caption={`${s.confirmedPeople} de ${s.people} personas`}
          />
        </section>
